...
func GenerateICS(event *openai.Event, timezone string) ([]byte, error) {
	...
	if event.StartTime.Hour() == 0 && event.StartTime.Minute() == 0 && event.StartTime.Second() == 0 {
		...
		startBefore := fmt.Sprintf("DTSTART:%s", event.StartTime.Format("20060102T150405Z"))
		startAfter := fmt.Sprintf("DTSTART;VALUE=DATE:%s", event.StartTime.Format("20060102"))
		...
		if event.EndTime.Hour() == 0 && event.EndTime.Minute() == 0 && event.EndTime.Second() == 0 {
			endBefore := fmt.Sprintf("DTEND:%s", event.EndTime.Format("20060102T150405Z"))
			endAfter := fmt.Sprintf("DTEND;VALUE=DATE:%s", event.EndTime.Format("20060102"))
